...
public class NetworkCreationRequestFactory {
    ...
    private Optional<String> getResourceGroupName(EnvironmentDto environmentDto) {
        return Optional.of(environmentDto).map(EnvironmentDto::getParameters).map(ParametersDto::getAzureParametersDto).map(AzureParametersDto::getAzureResourceGroupDto).map(AzureResourceGroupDto::getName);
    }
